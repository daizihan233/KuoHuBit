...
async def my_wf(app: Ariadne, group: Group, event: GroupMessage):
    ...
    if data:  # 如果在数据库中
        if event.sender.id not in ban_cache and not data[7]:
            ...
            result = await botfunc.select_fetchone(get_data_sql, (event.sender.id,))
        ...
        if event.sender.id not in forever_ban_cache and event.sender.id not in details_cache:
            ...
            if np.log10(data[4]) >= 1:
                ...
                data[4] = 0
            await botfunc.run_sql("UPDATE woodenfish SET de = %s, e = %s, ee = %s WHERE uid = %s",
        ...
        if result:
            if not result[7]:
                ...
                if int(time.time()) - res[9] <= botfunc.get_config('count_ban') and 5 <= res[10]:
                    ...
                    await botfunc.run_sql("UPDATE woodenfish SET de = de + %s WHERE uid = %s", (rad, event.sender.id))
            else:
                ban_cache.append(event.sender.id)
                await app.send_message(
                    group,
            ...
            if not result[7]:
                ...
                if int(time.time()) - res[9] <= botfunc.get_config('count_ban') and 5 <= res[10]:
                    ...
                    if result[4] >= result[2] + 2:
                        await botfunc.run_sql("UPDATE woodenfish SET e = e-level+2, level = level+1 WHERE uid = %s",
                                              (event.sender.id,))
                        await app.send_message(
                            group,
                            "木鱼升級成功辣！（喜）"
                        ...
                        await botfunc.run_sql("UPDATE woodenfish SET level = level+1, e = 0, ee = ee-%s WHERE uid = %s",
                        ...
                        if result[4] >= result[2] + 2:
                            await botfunc.run_sql("UPDATE woodenfish SET level = level+1, e = e-level+2 WHERE uid = %s",
                                                  (event.sender.id,))
                            await app.send_message(
                                group,
                                "木魚升級成功辣！（喜）"
                            ...
                            await botfunc.run_sql(
                                "UPDATE woodenfish SET level = level+1, e = 0, ee = ee-%s WHERE uid = %s",
                                (np.log10(result[2] + 2 - result[4]), event.sender.id))
                            await app.send_message(
                                group,
                                "木魚升級成功辣！（喜）"
